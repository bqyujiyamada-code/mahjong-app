  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (data.length > 0) {
        ...
        calculateRanking(data, uniqueSeasons[0]);
      }
      setLoading(false);
    }
    fetchData();
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
          ...
          <select 
            ...
            className="bg-white border border-gray-300 rounded-xl px-4 py-2 shadow-sm focus:ring-2 focus:ring-blue-500 outline-none text-sm font-bold"
          >
            {seasons.map(s => (
            ...
            <tbody className="divide-y divide-gray-100">
              {ranking.map((user, index) => (
                <tr key={user.name} className="hover:bg-blue-50 transition-colors">
                  ...
                  {/* 対局数（スリム化） */}
                  <td className="py-4 px-1 text-center">
                    <span className="inline-block bg-gray-100 text-gray-600 px-2 py-1 rounded-lg text-[10px] font-bold whitespace-nowrap border border-gray-200">
